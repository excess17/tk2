...
describe('Main', () => {
  ...
  describe('Details widget', () => {
    ...
    it('should display the right values', () => {
      cy.get(detailsTitle).should('be.visible').should('have.text', "Details about 'Nodi'");
      cy.get(entityIdCell).should('not.be.empty');
      cy.contains('entities.nodi.id').should('be.visible');
      cy.contains('entities.nodi.field1').should('be.visible');
    });
  });
});
